...
def clean_dataframe(
    ...
    # ── 1. Parse CSV ─────────────────────────────────────────
    # parse_bpom_csv sudah menangani:
    #   - BPOM format khusus (quoted rows + ;; terminator)
    #   - Rename kolom ke nama standar (tujuan_penyaluran, dll)
    #   - Fix dtype jumlah → float
    df        = parse_bpom_csv(file_source)
    before_df = df.copy()

    # Simpan nama kolom sebelum pipeline (untuk hitung renamed_columns)
    ...
    # ── 2. Query DataMatchModelAlamat dari PostgreSQL ─────────
    db_records = None
    try:
    ...
    # ── 3. Jalankan pipeline 6 langkah ───────────────────────
    # PENTING: karena csv_parser sudah menormalkan nama kolom,
    # run_pipeline STEP 1 (normalisasi kolom) akan menerima
    # kolom yang sudah benar (snake_case). Kolom yang sudah
    # sesuai tidak akan berubah.
    ...
    # Hitung berapa kolom yang namanya berubah setelah pipeline
    cols_after = list(cleaned_df.columns)
    summary["renamed_columns"] = sum(
        o != n
        for o, n in zip(cols_before, cols_after[:len(cols_before)])
    )

    # ── 4. Pastikan kolom wajib ada ──────────────────────────
    if "jenis_sarana" not in cleaned_df.columns:
        cleaned_df["jenis_sarana"] = "Lainnya"
    if "kategori_obat" not in cleaned_df.columns:
        cleaned_df["kategori_obat"] = kategori_jenis_obat or "General"

    # Isi kategori_obat kosong dengan fallback dari input user
    if kategori_jenis_obat:
        _empty_vals = {"", "nan", "none", "null", "lainnya", "general"}
        mask_empty_kat = (
            cleaned_df["kategori_obat"].isna() |
            cleaned_df["kategori_obat"].astype(str).str.strip().str.lower().isin(_empty_vals)
        )
        if mask_empty_kat.any():
            cleaned_df.loc[mask_empty_kat, "kategori_obat"] = kategori_jenis_obat

    # ── 5. Statistik jenis sarana ─────────────────────────────
    ...
    # ── 9. Drop baris jenis transaksi bukan "Dalam Negeri" ────
    # dan baris dengan keterangan "transaksi nihil" (case/space
    # insensitive), mengikuti aturan cleaning notebook:
    #
    #   df_clean = df_clean[
    #       (df_clean["Jumlah"] != 0) &
    #       (df_clean["Jenis Transaksi"] == "Dalam Negeri") &
    #       (df_clean["Keterangan"].str.strip().str.lower() != "transaksi nihil")
    #   ]
    #
    # Catatan: filter "Jumlah != 0" TIDAK diulang di sini karena
    # Rule 2 di anomaly_service.py sudah menandai jumlah<=0 sebagai
    # anomali (bukan di-drop) — supaya baris itu tetap terlihat di
    # tabel/preview untuk ditinjau user, bukan hilang diam-diam saat
    # cleaning. Dua filter baru di bawah ini MEMANG men-drop baris
    # (bukan menandai), sesuai permintaan eksplisit.
    rows_before_txn_filter = len(cleaned_df)

    if "jenis_transaksi" in cleaned_df.columns:
        mask_dalam_negeri = (
            cleaned_df["jenis_transaksi"].astype(str).str.strip().str.lower()
            == "dalam negeri"
        )
        n_dropped_jenis_transaksi = int((~mask_dalam_negeri).sum())
        cleaned_df = cleaned_df[mask_dalam_negeri]
    else:
        n_dropped_jenis_transaksi = 0

    if "keterangan" in cleaned_df.columns:
        mask_bukan_nihil = (
            cleaned_df["keterangan"].astype(str).str.strip().str.lower()
            != "transaksi nihil"
        )
        n_dropped_transaksi_nihil = int((~mask_bukan_nihil).sum())
        cleaned_df = cleaned_df[mask_bukan_nihil]
    else:
        n_dropped_transaksi_nihil = 0

    summary["dropped_jenis_transaksi"] = n_dropped_jenis_transaksi
    summary["dropped_transaksi_nihil"] = n_dropped_transaksi_nihil
    summary["rows_after_txn_filter"]   = rows_before_txn_filter - len(cleaned_df)
    ...
    process_log += (
        ...
        f"  Dropped (bukan Dalam Negeri): {summary['dropped_jenis_transaksi']:,}\n"
        f"  Dropped (transaksi nihil)   : {summary['dropped_transaksi_nihil']:,}\n"
